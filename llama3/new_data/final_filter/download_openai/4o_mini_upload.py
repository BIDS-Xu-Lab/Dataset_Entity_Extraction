...
def uplo(name):
  ...
  batch_input_file = client.files.create(
    file=open(f"/home/gy237/project/llama3/new_data/final_filter/upload_openai/{name}", "rb"),
    purpose="batch"
  )
...
folder_path = '/home/gy237/project/llama3/new_data/final_filter/upload_openai'
...
for i in a:
  ...
  des_list.append({'description':description, 'status':status, 'output':output_file_id})


des_list = [i for i in des_list if i['description'].endswith('jsonl') and i['status']=='completed']
print(len(des_list))
# print(des_list)
import os
for i in des_list:
...
# # Cancelling a Batch
# client.batches.cancel("batch_iz05dSVzUaasEglDsZnGceeO")
