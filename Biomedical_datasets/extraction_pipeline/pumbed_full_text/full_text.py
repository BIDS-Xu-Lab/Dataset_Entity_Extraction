...
def parse_xml_with_beautifulsoup(xml_content):
    ...
    extracted_data['journal-title'] = soup.find('journal-title').text if soup.find('journal-title') else None
    extracted_data['pmid'] = soup.find('article-id', {'pub-id-type': 'pmid'}).text if soup.find('article-id', {'pub-id-type': 'pmid'}) else None
    extracted_data['pmc'] = soup.find('article-id', {'pub-id-type': 'pmc'}).text if soup.find('article-id', {'pub-id-type': 'pmc'}) else None
    extracted_data['doi'] = soup.find('article-id', {'pub-id-type': 'doi'}).text if soup.find('article-id', {'pub-id-type': 'doi'}) else None
    extracted_data['article-title'] = soup.find('article-title').text if soup.find('article-title') else None
    extracted_data['abstract'] = soup.find('abstract').text if soup.find('abstract') else None
    ...
    if body:
        ...
        for section in sections:
            ...
            if 'introduction' in section.find('title').text.lower():
                ...
                extracted_data['introduction'] = ' '.join(p.prettify() for p in section.find_all('p'))
    ...
    for ref in refs:
        ...
        pmid = ref.find('pub-id', attrs={'pub-id-type': 'pmid'}).text if ref.find('pub-id', attrs={'pub-id-type': 'pmid'}) else None
        title = ref.find('article-title').text if ref.find('article-title') else None
        authors = [' '.join([name.text for name in author.find_all(['surname', 'given-names'])]) for author in ref.find_all('name')]
        extracted_data['references'].append({'rid': rid, 'pmid': pmid, 'title': title, 'authors': authors})
