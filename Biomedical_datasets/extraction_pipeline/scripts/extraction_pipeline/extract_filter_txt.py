...
def parse_das_sec(pmc_file_path):
    ...
    if statement_node is not None:
        ...
        das_statement = ""
        das_links = []
    das_sec = {"das_title": das_title, "das_statement": das_statement.strip(), "das_links": das_links}
    return das_sec


def parse_all_sec(pmc_file_path):
    das_sec = parse_das_sec(pmc_file_path)
    all_sections = parse_sec_text(pmc_file_path) + [{'section': 'DAS', 'text': das_sec['das_statement']}]
    return all_sections
...
def search_intro_sec(pmc_file_path, search_term):
    ...
    filtered_sec_text = []
    for sec in all_sections:
        if 'introduction' in sec['section'].lower() or 'background' in sec['section'].lower():
            if re.search(search_term, sec['text']):
