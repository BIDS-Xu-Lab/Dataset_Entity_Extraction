...
_list = []
for name in ['genbank_pmcid', 'gene_expression_omnibus_pmcid', 'pubchem_pmcid', 'protein_data_bank_pmcid', 'uniprot_pmcid']:
    _list += sample(name, 5)
print(len(_list))
...
for name in all_items:
    if name not in ['clinical_trials_pmcid', 'genbank_pmcid', 'gene_expression_omnibus_pmcid', 'pubchem_pmcid', 'protein_data_bank_pmcid', 'uniprot_pmcid']:
        _list += sample(name, 3)
...
import csv
with open('/home/gy237/project/Biomedical_datasets/extraction_pipeline/esearch/gui_50.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    for item in _list[:50]:
        writer.writerow([item])  # 每个元素作为一行写入
with open('/home/gy237/project/Biomedical_datasets/extraction_pipeline/esearch/kalpana_50.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    for item in _list[50:]:
        writer.writerow([item])  # 每个元素作为一行写入
