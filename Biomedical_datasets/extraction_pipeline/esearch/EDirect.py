...
def get_pmids(query, total_records):
    ...
    while fetched_records < total_records:
        esearch_command = [
            ...
            "|", "efetch", "-format", "uid"# , "-start", str(fetched_records + 1), "-stop", str(fetched_records + batch_size)
        ]
        # time.sleep(3)
        # print(" ".join(esearch_command))

        # 执行命令
        result = subprocess.run(" ".join(esearch_command), shell=True, capture_output=True, text=True)
...
def get_pmcids(pmid_list):
    ...
    for i in range(0, len(pmid_list), batch_size):
        ...
        elink_command = [
            "elink", "-db", "pubmed", "-db", "pmc", "-target", "pmc", "-id", ",".join(pmid_batch),
            "|", "efetch", "-format", "uid"
        ]


        result = subprocess.run(" ".join(elink_command), shell=True, capture_output=True, text=True)
        pmcid_batch = result.stdout.strip().split('\n')
        pmcid_list.extend(pmcid_batch)
        print(f"已获取 {len(pmcid_batch)} 个 PMCID，总共已获取 {len(pmcid_list)} 个 PMCID")
            ...
            for linkset in data['linksets']:
                if 'linksetdbs' in linkset:
                    for linksetdb in linkset['linksetdbs']:
                        if linksetdb['dbto'] == "pmc" and linksetdb['linkname'] == 'pubmed_pmc' and 'links' in linksetdb:
...
with open(f'/home/gy237/project/Biomedical_datasets/extraction_pipeline/esearch/{repo_name}_pmid.txt', 'w') as f:
...
with open(f'/home/gy237/project/Biomedical_datasets/extraction_pipeline/esearch/{repo_name}_pmcid.txt', 'w') as f:
