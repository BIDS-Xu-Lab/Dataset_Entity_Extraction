...
if add_token:
    tokenizer = BertTokenizerFast.from_pretrained(f'{ner_imfile}/tokenizer_file', do_lower_case=do_lower_case)
    print('Finish loading NER_tokenizer')
else:
    tokenizer = BertTokenizerFast.from_pretrained(model_name, do_lower_case=do_lower_case, local_files_only=True)
...
if re_add_token:
    re_tokenizer = BertTokenizerFast.from_pretrained(f'{re_imfile}/tokenizer_file', do_lower_case=re_do_lower_case)
    print('Finish loading RE_tokenizer')
else:
    re_tokenizer = BertTokenizerFast.from_pretrained(re_model_name, do_lower_case=re_do_lower_case, local_files_only=True)
...
with open('/home/gy237/project/Biomedical_datasets/data_update/precessed_data/test_data.json', 'r', encoding='utf-8') as file:
...
def process_data(sentence, text_labels):
    ...
    sen_input_ids = pad_sequences([tokenizer.convert_tokens_to_ids(tokenized_sentence)],
                          maxlen=MAX_LEN, dtype="long", value=0.0,
                          truncating="post", padding="post")

    sen_tags = pad_sequences([[tag2idx.get(l) for l in labels]],
...
with open('/home/gy237/project/Biomedical_datasets/combined/special_characters.json', 'r', encoding='utf-8') as file:
...
def model_process(sen_dataloader):
    for batch in sen_dataloader:
        ...
        pred_tags = [idx2tag[p_i] for p, l in zip(preds_ids, label_ids) for p_i, l_i in zip(p, l) if idx2tag[l_i] != "PAD"]
        valid_tags = [idx2tag[l_i] for l in label_ids for l_i in l if idx2tag[l_i] != "PAD"]
...
for idx in range(len(devel_sen)):
    ...
    if idx_list == []:
        ...
        for ix in range(len(idx_list) - 1):
            sen_dataloader = process_data(sen_texts[idx_list[ix]:idx_list[ix+1]], sen_labs[idx_list[ix]:idx_list[ix+1]])
            ...
            if ix == len(idx_list) - 2:
                sen_dataloader = process_data(sen_texts[idx_list[ix+1]:], sen_labs[idx_list[ix+1]:])
                _text, _pred_tags, _valid_tags = model_process(sen_dataloader)
                text.extend(_text)
                pred_tags.extend(_pred_tags)
                valid_tags.extend(_valid_tags)
