...
# load data
if no_spacy:
    with open('/home/gy237/project/Biomedical_datasets/data_update/precessed_data/train_data.json', 'r', encoding='utf-8') as file:
    ...
    # load data
    with open('/home/gy237/project/Biomedical_datasets/NER/data/train_data.json', 'r', encoding='utf-8') as file:
        train_data = json.load(file)
        #train_data = [ json.load(line) for line in data_list ]
    train_sentences = [i["sen"] for i in train_data]
    train_labels = [i["lab"] for i in train_data]

    with open('/home/gy237/project/Biomedical_datasets/NER/data/test_data.json', 'r', encoding='utf-8') as file:
        test_data = json.load(file)
    test_sentences = [i["sen"] for i in test_data]
    test_labels = [i["lab"] for i in test_data]
...
tr_inputs, tr_tags, tr_masks= shuffle(train_input_ids, train_tags, train_attention_masks, random_state=SEED)
val_inputs, val_tags, val_masks = shuffle(test_input_ids, test_tags, test_attention_masks, random_state=SEED)
...
for epoch in trange(epochs, desc="Epoch"):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if use_fgm:
            ...
            loss_sum = outputs[0]
            if loss_weights:
                loss_sum = loss_fn(outputs[1].view(-1, outputs[1].size(-1)), b_labels.view(-1))
    ...
    # save something
    performance_df = pd.DataFrame({ 'validation_accuracy': validation_accuracy,
                                    'validation_f1_weighted': validation_f1,
                                    'validation_f1_macro': validation_f2})
    performance_df.to_csv(performance_file, index=False)

    epoch_loss_df = pd.DataFrame({ 'train_loss_values': train_loss_values,
